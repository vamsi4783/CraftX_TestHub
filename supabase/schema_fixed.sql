-- =======================================================
-- TestHub QA Platform — Complete PostgreSQL Schema (FIXED)
-- FIX: auth.uid() cast to ::uuid in all RLS policies and functions
-- =======================================================
...
-- Bug history (FIXED: explicit ::text casts)
CREATE OR REPLACE FUNCTION track_bug_changes()
RETURNS TRIGGER LANGUAGE plpgsql SECURITY DEFINER AS $$
BEGIN
    IF OLD.status   != NEW.status   THEN INSERT INTO bug_history(bug_id,changed_by,field_name,old_value,new_value) VALUES(NEW.id,(auth.uid())::uuid,'status',OLD.status::TEXT,NEW.status::TEXT); END IF;
    IF OLD.severity != NEW.severity THEN INSERT INTO bug_history(bug_id,changed_by,field_name,old_value,new_value) VALUES(NEW.id,(auth.uid())::uuid,'severity',OLD.severity::TEXT,NEW.severity::TEXT); END IF;
    IF OLD.priority != NEW.priority THEN INSERT INTO bug_history(bug_id,changed_by,field_name,old_value,new_value) VALUES(NEW.id,(auth.uid())::uuid,'priority',OLD.priority::TEXT,NEW.priority::TEXT); END IF;
    IF OLD.assigned_to IS DISTINCT FROM NEW.assigned_to THEN INSERT INTO bug_history(bug_id,changed_by,field_name,old_value,new_value) VALUES(NEW.id,(auth.uid())::uuid,'assigned_to',OLD.assigned_to::TEXT,NEW.assigned_to::TEXT); END IF;
    RETURN NEW;
END;
$$;
...
-- Helper functions (FIXED: explicit (auth.uid())::uuid cast everywhere)
CREATE OR REPLACE FUNCTION is_project_member(p UUID) RETURNS BOOLEAN LANGUAGE plpgsql SECURITY DEFINER AS $$
BEGIN
    RETURN EXISTS(SELECT 1 FROM project_members WHERE project_id=p AND user_id=(auth.uid())::uuid)
        OR EXISTS(SELECT 1 FROM projects WHERE id=p AND owner_id=(auth.uid())::uuid)
        OR EXISTS(SELECT 1 FROM profiles WHERE id=(auth.uid())::uuid AND role='administrator'::user_role);
END;$$;

CREATE OR REPLACE FUNCTION is_admin() RETURNS BOOLEAN LANGUAGE plpgsql SECURITY DEFINER AS $$
BEGIN RETURN EXISTS(SELECT 1 FROM profiles WHERE id=(auth.uid())::uuid AND role='administrator'::user_role); END;$$;
