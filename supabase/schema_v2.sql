-- =======================================================
-- TestHub QA Platform — Schema v2
-- FIX: gen_random_uuid() replaces uuid_generate_v4() (PostgreSQL 13+ built-in)
-- FIX: explicit enum::type casts on all DEFAULT values
-- FIX: (auth.uid())::uuid explicit cast everywhere
-- =======================================================
...
CREATE TABLE releases (
    ...
    build_number  TEXT,
    status        release_status NOT NULL DEFAULT 'planning'::release_status,
    start_date    DATE,
    end_date      DATE,
    release_notes TEXT,
    known_issues  TEXT,
...
CREATE TABLE test_cases (
    ...
    title               TEXT NOT NULL,
    description         TEXT,
    priority            tc_priority NOT NULL DEFAULT 'medium'::tc_priority,
    status              tc_status NOT NULL DEFAULT 'draft'::tc_status,
    estimated_minutes   INTEGER NOT NULL DEFAULT 15,
    is_automation_ready BOOLEAN NOT NULL DEFAULT FALSE,
    preconditions       TEXT,
    tags                TEXT[] NOT NULL DEFAULT '{}',
    created_by          UUID NOT NULL REFERENCES profiles(id),
...
CREATE TABLE bugs (
    ...
    title              TEXT NOT NULL,
    description        TEXT NOT NULL,
    severity           bug_severity NOT NULL DEFAULT 'medium'::bug_severity,
    priority           bug_priority NOT NULL DEFAULT 'p2'::bug_priority,
    status             bug_status NOT NULL DEFAULT 'new'::bug_status,
...
CREATE TABLE attachments (
    ...
    file_kind   attachment_kind NOT NULL DEFAULT 'other'::attachment_kind,
    mime_type   TEXT,
    file_size   BIGINT,
    uploaded_by UUID NOT NULL REFERENCES profiles(id),
    created_at  TIMESTAMPTZ NOT NULL DEFAULT NOW()
...
CREATE TABLE feature_requests (
    ...
    business_value TEXT,
    category       TEXT,
    priority       feature_priority NOT NULL DEFAULT 'medium'::feature_priority,
    status         feature_status NOT NULL DEFAULT 'submitted'::feature_status,
...
-- Alias so migrations 002/003 work without change
...
BEGIN
    v_uid := (auth.uid())::uuid;
    IF OLD.status    != NEW.status    THEN INSERT INTO bug_history(bug_id, changed_by, field_name, old_value, new_value) VALUES (NEW.id, v_uid, 'status',      OLD.status::TEXT,      NEW.status::TEXT);      END IF;
    IF OLD.severity  != NEW.severity  THEN INSERT INTO bug_history(bug_id, changed_by, field_name, old_value, new_value) VALUES (NEW.id, v_uid, 'severity',    OLD.severity::TEXT,    NEW.severity::TEXT);    END IF;
    IF OLD.priority  != NEW.priority  THEN INSERT INTO bug_history(bug_id, changed_by, field_name, old_value, new_value) VALUES (NEW.id, v_uid, 'priority',    OLD.priority::TEXT,    NEW.priority::TEXT);    END IF;
    IF OLD.assigned_to IS DISTINCT FROM NEW.assigned_to THEN INSERT INTO bug_history(bug_id, changed_by, field_name, old_value, new_value) VALUES (NEW.id, v_uid, 'assigned_to', OLD.assigned_to::TEXT, NEW.assigned_to::TEXT); END IF;
    RETURN NEW;
END;
$$;
...
BEGIN
    ...
    SELECT COUNT(*) INTO completed_tests FROM test_assignments WHERE release_id = p_release_id AND status = 'completed'::assignment_status;
    SELECT COUNT(*) INTO passed_tests    FROM test_results     WHERE release_id = p_release_id AND status = 'pass'::result_status;
    SELECT COUNT(*) INTO critical_bugs   FROM bugs WHERE release_id = p_release_id AND severity = 'critical'::bug_severity AND status NOT IN ('closed'::bug_status, 'rejected'::bug_status, 'duplicate'::bug_status);
    SELECT COUNT(*) INTO open_bugs       FROM bugs WHERE release_id = p_release_id AND status IN ('new'::bug_status, 'triaged'::bug_status, 'assigned'::bug_status, 'in_progress'::bug_status);
    SELECT COUNT(*) INTO total_bugs      FROM bugs WHERE release_id = p_release_id AND status NOT IN ('closed'::bug_status, 'rejected'::bug_status, 'duplicate'::bug_status);
    ...
    verdict     := CASE
        WHEN critical_bugs > 0 OR testing_pct < 50                    THEN 'not_ready'::readiness_verdict
        WHEN open_bugs > 0     OR testing_pct < 80 OR pass_rate < 90  THEN 'ready_with_risks'::readiness_verdict
        ELSE 'ready_for_release'::readiness_verdict
    END;
...
BEGIN
    v_uid := (auth.uid())::uuid;
    RETURN EXISTS(SELECT 1 FROM project_members WHERE project_id = p AND user_id = v_uid)
        OR EXISTS(SELECT 1 FROM projects        WHERE id = p AND owner_id = v_uid)
        OR EXISTS(SELECT 1 FROM profiles        WHERE id = v_uid AND role = 'administrator'::user_role);
END;
$$;
...
BEGIN
    v_uid := (auth.uid())::uuid;
    RETURN EXISTS(SELECT 1 FROM profiles WHERE id = v_uid AND role = 'administrator'::user_role);
END;
$$;

CREATE POLICY "prof_sel" ON profiles FOR SELECT TO authenticated USING (TRUE);
CREATE POLICY "prof_upd" ON profiles FOR UPDATE TO authenticated USING (id = (auth.uid())::uuid OR is_admin());

CREATE POLICY "proj_sel" ON projects FOR SELECT TO authenticated USING (is_project_member(id) OR is_admin());
CREATE POLICY "proj_ins" ON projects FOR INSERT TO authenticated WITH CHECK (owner_id = (auth.uid())::uuid);
CREATE POLICY "proj_upd" ON projects FOR UPDATE TO authenticated USING (owner_id = (auth.uid())::uuid OR is_admin());
CREATE POLICY "proj_del" ON projects FOR DELETE TO authenticated USING (owner_id = (auth.uid())::uuid OR is_admin());
...
CREATE POLICY "pm_ins" ON project_members FOR INSERT TO authenticated WITH CHECK (
    EXISTS(SELECT 1 FROM projects WHERE id = project_id AND owner_id = (auth.uid())::uuid) OR is_admin());
CREATE POLICY "pm_del" ON project_members FOR DELETE TO authenticated USING (
    EXISTS(SELECT 1 FROM projects WHERE id = project_id AND owner_id = (auth.uid())::uuid) OR is_admin());

CREATE POLICY "rel_all"    ON releases          FOR ALL TO authenticated USING (is_project_member(project_id)) WITH CHECK (is_project_member(project_id));
CREATE POLICY "reldoc_all" ON release_documents FOR ALL TO authenticated USING (EXISTS(SELECT 1 FROM releases r WHERE r.id = release_id AND is_project_member(r.project_id)));
...
CREATE POLICY "tcs_all"  ON test_case_steps  FOR ALL TO authenticated USING (EXISTS(SELECT 1 FROM test_cases t WHERE t.id = test_case_id AND is_project_member(t.project_id)));
CREATE POLICY "ta_all"   ON test_assignments FOR ALL TO authenticated USING (EXISTS(SELECT 1 FROM test_cases t WHERE t.id = test_case_id AND is_project_member(t.project_id)));
CREATE POLICY "tr_all"   ON test_results     FOR ALL TO authenticated USING (EXISTS(SELECT 1 FROM test_cases t WHERE t.id = test_case_id AND is_project_member(t.project_id)));
CREATE POLICY "trsr_all" ON test_result_step_results FOR ALL TO authenticated USING (EXISTS(SELECT 1 FROM test_results r WHERE r.id = test_result_id));
...
CREATE POLICY "att_all" ON attachments FOR ALL TO authenticated USING (uploaded_by = (auth.uid())::uuid OR is_admin());
...
CREATE POLICY "frv_all" ON feature_request_votes    FOR ALL TO authenticated USING (EXISTS(SELECT 1 FROM feature_requests f WHERE f.id = feature_request_id AND is_project_member(f.project_id)));
CREATE POLICY "frc_all" ON feature_request_comments FOR ALL TO authenticated USING (EXISTS(SELECT 1 FROM feature_requests f WHERE f.id = feature_request_id AND is_project_member(f.project_id)));

CREATE POLICY "notif_own" ON notifications FOR ALL TO authenticated USING (user_id = (auth.uid())::uuid);
...
CREATE POLICY "act_ins"   ON activity_logs FOR INSERT TO authenticated WITH CHECK (user_id = (auth.uid())::uuid);
